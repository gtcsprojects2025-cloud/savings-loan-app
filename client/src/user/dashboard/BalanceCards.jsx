...
import { BanknotesIcon, CurrencyDollarIcon, DocumentTextIcon } from '@heroicons/react/24/outline';

const BalanceCards = ({ balances }) => {
  const cards = [
    { title: 'Account Balance', amount: balances.account, icon: BanknotesIcon, color: 'bg-brandBlue' },
      ...
      {cards.map((card) => (
        ...
        >
          ...
          <div>
            ...
            <p className="text-2xl font-bold text-black">N{card.amount.toLocaleString()}</p>
          </div>
        </div>
      ))}
    </div>
